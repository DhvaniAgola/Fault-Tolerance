...
public class Server2
{
	...
	{
		Logger logs=Logger.getLogger("ServerSOEN");
		FileHandler FH = new FileHandler("ServerSOEN.log", true);
		logs.addHandler(FH);
		Impl impl = new Impl("SOEN",logs);
		new Thread(() -> {
			try {
				UDP(impl);
		...
		try
		{
			aSocket = new MulticastSocket(2222);
			aSocket.joinGroup(InetAddress.getByName("230.1.1.5"));
			logs.info("Replica2 running...");
			while (true)
			{
