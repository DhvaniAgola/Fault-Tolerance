...
public class Server1
{
	...
	public synchronized static void main(String[] args) throws ClassNotFoundException, SecurityException, IOException
	{
		Logger logs = Logger.getLogger("ServerCOMP");
		FileHandler FH = new FileHandler("ServerCOMP.log", true);
		logs.addHandler(FH);
		Impl impl = new Impl("COMP", logs);
		new Thread(() -> {
			try {
				UDP(impl);
		...
		try
		{
			aSocket = new MulticastSocket(1111);
			...
			while (true)
			{
				...
				if(data.operationID == 10) {					
					DatagramPacket reply = new DatagramPacket(responseMessage, responseMessage.length, InetAddress.getByName("localhost"), request.getPort());
					aSocket.send(reply);// reply sent
				} else {
					strReply = "1#" + strReply;
					System.out.println("strReply 142 : "+strReply);
					DatagramPacket reply = new DatagramPacket(strReply.getBytes(), strReply.getBytes().length, InetAddress.getByName(data.ipAddress), 1234);
					aSocket.send(reply);// reply sent
				}
